...
const makePlayer = () =>
    Player.create({ identityId: VALID_IDENTITY_ID, name: VALID_NAME }).asArray()[0] as Player;
...
describe('PlayerController', () => {
    ...
    describe('POST /players', () => {
        it('deve retornar o player criado', async () => {
            ...
            const result = await controller.create({ identityId: VALID_IDENTITY_ID, name: VALID_NAME });
        ...
        it('deve lançar BadRequestException para identity ID inválido', async () => {
            ...
            await expect(
                controller.create({ identityId: '', name: VALID_NAME }),
        ...
        it('deve lançar BadRequestException para nome inválido', async () => {
            ...
            await expect(
                controller.create({ identityId: VALID_IDENTITY_ID, name: 'ab' }),
            ).rejects.toBeInstanceOf(BadRequestException);
        });
        ...
        it('deve lançar ConflictException quando player já existe', async () => {
            ...
            await expect(
                controller.create({ identityId: VALID_IDENTITY_ID, name: VALID_NAME }),
