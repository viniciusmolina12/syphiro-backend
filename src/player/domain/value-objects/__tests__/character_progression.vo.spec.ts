import { CHARACTER_PROGRESSION_RULES, CharacterProgression, InvalidCharacterProgressionError } from "../character_progression.vo";

describe('CharacterProgression Value Object', () => {
    it('should be able to create a character progression', () => {
        const [characterProgression, error] = CharacterProgression.create(6401).asArray();
    ...
    it('should be able to gain experience', () => {
        const [characterProgression, _] = CharacterProgression.create(6400).asArray();
    ...
    it('should not be able to create a character progression with negative experience', () => {
        const [_, error] = CharacterProgression.create(-100).asArray();
        expect(error).toBeInstanceOf(InvalidCharacterProgressionError);
        expect(error.message).toBe(CHARACTER_PROGRESSION_RULES.MIN_EXPERIENCE.message);
    });

    it('should be able to level up', () => {
        const [characterProgression, _] = CharacterProgression.create(6300).asArray();
